...
originales <- bind_cols(list(Name = names, Originales = sources))
traducciones <- left_join(datos, originales) %>% 
  mutate(Originales = case_when(Name == "datos-stringr" ~ "incluidos en el paquete stringr", TRUE ~ as.character(Originales))) %>% 
  mutate(Originales = str_replace_all(Originales, 'tibble::rownames_to_column\\(datasets::mtcars, \\\"model\\\"\\)', "datasets::mtcars"))
knitr::kable(stats::setNames(traducciones, c("Nombre", "Título", "Dataset original")))
```
